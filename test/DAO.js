...
describe('DAO', () => {
  ...
  beforeEach(async () => {
   ...
   transaction  = await token.connect(deployer).transfer(investor2.address, tokens(200000))
    ...
    // 500k tokens + 1 wei, i.e., 5000000000000000000000001
    const DAO = await ethers.getContractFactory('DAO')
    dao = await DAO.deploy(token.address, '5000000000000000000000001') 
  ...
  describe('Deployment', () => {
    it('sends either to the DAO treasury', async () => {
    ...
    it('returns quorum', async () => {
      expect(await dao.quorum()).to.equal('5000000000000000000000001')
    })
